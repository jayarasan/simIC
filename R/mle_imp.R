...
mle_imp <- function(left, right, dist = "weibull",
  ...
  neg_loglik_init <- switch(dist,
                            ...
                            "gamma" = function(par) -sum(dgamma(prelim_times, par[1], par[2], log = TRUE)),
  ...
  quantile_fn <- switch(dist,
                        ...
                        "gompertz" = function(p) log(1 - log(1 - p) * (est_par[1] / est_par[2])) / est_par[1],
  ...
  for (i in seq_along(left)) {
    if (!is.finite(right[i])) {
      ...
      x <- runif(100, left[i], right[i])
      times[i] <- median(x)
    } else if (impute == "harmonic_median") {
      x <- runif(100, left[i], right[i])
      times[i] <- 1 / median(1 / x)
    } else if (impute == "geometric_median") {
      x <- runif(100, left[i], right[i])
      times[i] <- exp(median(log(x)))
  ...
  neg_loglik_final <- switch(dist,
                             ...
                             "gamma" = function(par) -sum(dgamma(times, par[1], par[2], log = TRUE)),
                             "gompertz" = function(par) {
                               a <- par[1]; b <- par[2]
                               if (a <= 0 || b <= 0) return(Inf)
  ...
  fit <- optim(par = est_par, fn = neg_loglik_final, method = "L-BFGS-B", lower = rep(1e-5, length(est_par)), hessian = TRUE)
